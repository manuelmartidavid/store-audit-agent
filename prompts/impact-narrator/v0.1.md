...
could not be reached. Nothing else.

That summary is the entire deliverable, and it still has to be worth sending. Say
that the store could not be reached, say that no page could be inspected, and say
that no assessment was possible as a result. Do not:
...
- name or guess a cause for the block — a password gate, a firewall, a bot check,
  anything. You were not given one, and "could not be reached" is both accurate
  and enough
- describe the gate as a defect or a finding — it is a fact about access, not a
  cause, and the report does not need one to state it
...
6. Re-read every field you wrote. Check each one for digits — if you find one,
   rewrite the sentence directionally. Then check each one against its cap in
   the table above — if it runs over, cut words until it doesn't.
