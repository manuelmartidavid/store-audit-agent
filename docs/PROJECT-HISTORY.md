# Store Audit Agent — project history (archive)

    created:  2026-07-30, when PROJECT-STATE.md was trimmed to what the next
              steps actually need.
    status:   ARCHIVE. Not ground truth. `PROJECT-STATE.md` is.

This is the full text of `PROJECT-STATE.md` as it stood on 2026-07-30, immediately
before it was trimmed. Nothing here was deleted from the project — it was moved,
so the narrative record survives for anyone who needs the *why* behind a settled
decision, without the live file carrying 1,395 lines of it.

What was kept in `PROJECT-STATE.md`: the settled decisions in compressed form, the
current state, the next steps, the provenance pins, the open decision, and — at
full length — every gotcha that could resurface as a bug. What moved here: the
blow-by-blow of how each of those was arrived at (recaptures #1–#3, the step-7
prompt lineage, step 8's corrections, the full argumentation for decisions 28–31),
plus this file's own record of corrections to its earlier text.

Two cautions when reading:

- **Figures here date.** Several are explicitly superseded and say so; others are
  simply older than the live file. Where the two disagree, `PROJECT-STATE.md`
  wins.
- **`STALE-OK` markers are load-bearing.** They exempt a line from
  `tests/test_repo_hygiene.py`'s stale-path lint, which scopes this file too.
  Do not strip them.
