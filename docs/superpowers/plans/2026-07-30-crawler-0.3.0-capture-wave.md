...
- Every task ends green on `python -m pytest tests/ -q`. ~~The full suite is ~6m20s;~~ **measured 15m31s at 614 passed / 1 skipped once this wave had landed (2026-07-30).** The ~6m20s figure was taken before it and is superseded rather than wrong — this wave's own additions account for the gap: a real 2s `Crawl-delay` honoured across an integration capture, plus the WooCommerce storefront routes doubling the browser-driven discovery paths. Run the named file during a task and the full suite before the commit, and budget the wait.
